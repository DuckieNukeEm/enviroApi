...
class variable_units():
    ...
    variables: list = ["light", "temperature", "pressure", "humidity", "oxi", "redux", "nh3", "pm1", "pm2.5", "pm10", "noise", 'c02','voc']
    units: list = ["Lux", "C", "hPa", "%", "kOhms"<, "kOhms", "kOhms", "ug/m3", "ug/m3", "ug/m3", "dBa", "kOhms",'kOhms']
    light: str = self.variables[0]
    light_unit: str = units[0]
    temperature: str = self.variables[1]
    temperature_unit: str = units[1]
    pressure: str = self.variables[2]
    pressure_unit: str = units[2]
    humidity: str = self.variables[3]
    humidity_unit: str = units[3]
    oxidising: = self.variables[4]
    oxidising_unit: str = units[4]
    reducing: str = self.variables[5]
    reducing_unit:  str = units[5]
    nh3:  str = self.variables[6]
    nh3_unit:  str = units[6]
    pm1:  str = self.variables[7]
    pm1_unit:  str = units[7]
    pm25:  str = self.variables[8]
    pm25_unit:  str = units[8]
    pm10:  str = self.variables[9]
    pm10_unit:  str = units[9]
    noise:  str = self.variables[10]
    noise_unit:  str = units[10]
    co2: str = self.variable[11]
    co2_unit: str = self.units[11]
    voc: str = self.variable[12] 
    voc_unit: str = self.units[12]   
@dataclass
class Compensation():
...
def load_compensation(config: Config, path: [str, None]) -> dataclass:
    ...
    if Config.has_weather_cover is False:
        ...
        weather_dict['comp_temp_cub_d'] = weather_dict['comp_temp_cub_d'] + Config.Offset # Don't ask me why he does this
...
def retrieve_config(config_path: [str, None]) -> dataclass:
    ...
    config_dict['enable_display'] = config_json['enable_display'] # Enables the display and flags
    ...
    config_dict['enable_climate_and_gas_logging'] = config_json['enable_climate_and_gas_logging']
    ...
    config_dict['gas_daily_r0_calibration_hour'] = config_json.get('gas_daily_r0_calibration_hour', 3)
    config_dict['reset_gas_sensor_calibration'] = config_json.get('reset_gas_sensor_calibration', False)
